...
Checks whether `T` is a pointer to object or function (including pointer to
`void`, but excluding pointer to member) or a cv-qualified version thereof.
...
The behavior of a program that adds specializations for `std::is_pointer` or
`std::is_pointer_v` is undefined.
...
```cpp
template< class T >
inline constexpr bool is_pointer_v = is_pointer<T>::value;  // (since C++17)
```
