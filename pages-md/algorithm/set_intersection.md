...
Constructs a sorted range beginning at `d_first` consisting of elements that are
found in both sorted ranges `[``first1``,``last1``)` and
`[``first2``,``last2``)`.
...
them, the first `std::min(m, n)` elements will be copied from
