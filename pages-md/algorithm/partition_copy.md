...
1) Copies the elements from the range `[``first``,``last``)` to two different
   ranges depending on the value returned by the predicate `p`. The elements
   that satisfy the predicate `p` are copied to the range beginning at
   `d_first_true`. The rest of the elements are copied to the range beginning at
   `d_first_false`.
...
- **p** — unary predicate which returns ​`true` if the element should be placed
  in d_first_true. The expression `p(v)` must be convertible to `bool` for every
...
A `std::pair` constructed from the iterator to the end of the `d_first_true`
range and the iterator to the end of the `d_first_false` range.
