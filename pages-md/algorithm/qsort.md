...
Sorts the given array pointed to by `ptr` in ascending order. The array contains
`count` elements of `size` bytes. Function pointed to by `comp` is used for
object comparison.
...
- **comp** — comparison function which returns ​a negative integer value if the
  ...
  equivalent. The signature of the comparison function should be equivalent to
