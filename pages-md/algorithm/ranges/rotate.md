...
constexpr ranges::borrowed_subrange_t<R>
    rotate( R&& r, ranges::iterator_t<R> middle );  // (2) (since C++20)
```

1) Performs a *left rotation* on a range of elements. Specifically,
   `ranges::rotate` swaps the elements in the range `[``first``,``last``)` in
   such a way that the element `*middle` becomes the first element of the new
   range and `*(middle - 1)` becomes the last element.
