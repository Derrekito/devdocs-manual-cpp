...
1) Returns `true` if the projections of the sorted range
   `[``first2``,``last2``)` is a subsequence of the projections of the sorted
   range `[``first1``,``last1``)`.
...
At most \(\scriptsize 2 \cdot (N_1+N_2-1)\)2·(N1+N2-1) comparisons, where
\(\scriptsize N_1\)N1 is `ranges::distance(r1)` and \(\scriptsize N_2\)N2 is
`ranges::distance(r2)`.
...
---
*Source: https://en.cppreference.com/w/cpp/algorithm/ranges/includes*
