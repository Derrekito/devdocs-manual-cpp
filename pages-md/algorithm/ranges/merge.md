...
using merge_result = ranges::in_in_out_result<I1, I2, O>;  // (3) (since C++20)
```

Merges two *sorted* ranges `[``[first1``,``last1``)` and
`[``first2``,``last2``)` into one *sorted* range beginning at `result`.

A sequence is said to be *sorted* with respect to the comparator `comp` if for
any iterator `it` pointing to the sequence and any non-negative integer `n` such
...
This algorithm performs a similar task as `ranges::set_union` does. Both consume
...
`ranges::set_union` would output `max(n, m)` ones only. So `ranges::merge`
outputs exactly `N` values and `ranges::set_union` may produce fewer.
...
---
*Source: https://en.cppreference.com/w/cpp/algorithm/ranges/merge*
