...
using unique_copy_result = ranges::in_out_result<I, O>;  // (3) (since C++20)
```
...
1) Copies the elements from the source range `[``first``,``last``)`, to the
   ...
   consecutive equal elements. Only the first element of each group of equal
   elements is copied.

The ranges `[``first``,``last``)` and `[``result``,``result + N``)` must not
   overlap. `N = ranges::distance(first, last)`.
...
Exactly `N - 1` applications of the corresponding predicate `comp` and no more
than twice as many applications of any projection `proj`.
