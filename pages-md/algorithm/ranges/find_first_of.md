...
1) Searches the range `[``first1``,``last1``)` for *any* of the elements in the
   range `[``first2``,``last2``)`, after projecting the ranges with `proj1` and
...
equal to an element from the range `[``first2``,``last2``)` after projection. If
no such element is found, an iterator comparing equal to `last1` is returned.
