...
using set_intersection_result = ranges::in_in_out_result<I1, I2, O>;  // (3) (since C++20)
```

Constructs a sorted range beginning at `result` consisting of elements that are
found in both sorted input ranges `[``first1``,``last1``)` and
`[``first2``,``last2``)`. If some element is found `m` times in
`[``first1``,``last1``)` and `n` times in `[``first2``,``last2``)`, the first
`min(m, n)` elements will be copied from the first range to `result`. The order
...
---
*Source: https://en.cppreference.com/w/cpp/algorithm/ranges/set_intersection*
