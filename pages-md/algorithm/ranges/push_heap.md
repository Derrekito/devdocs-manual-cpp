...
constexpr I
    push_heap( I first, S last, Comp comp = {}, Proj proj = {} );  // (1) (since C++20)
...
Inserts the element at the position `last - 1` into the max heap defined by the
range `[``first``,``last - 1``)`.
...
\log{(N)}\)log(N) comparisons and \(\scriptsize 2\log{(N)}\)2log(N) projections.
