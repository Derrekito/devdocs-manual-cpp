...
using swap_ranges_result = ranges::in_in_result<I1, I2>;  // (3) (since C++20)
```

1) Exchanges elements between first range `[``first1``,``first1 + M``)` and
   second range `[``first2``,``first2 + M``)` via `ranges::iter_swap(first1 + i,
   first2 + i)`, where `M = ranges::min(ranges::distance(first1, last1),
   ranges::distance(first2, last2))`.

The ranges `[``first1``,``last1``)` and `[``first2``,``last2``)` must not
   overlap.
