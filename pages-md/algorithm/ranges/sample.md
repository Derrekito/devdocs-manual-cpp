...
O sample( I first, S last, O out, std::iter_difference_t<I> n, Gen&& gen );  // (1) (since C++20)
...
requires (ranges::forward_range<R> || std::random_access_iterator<O>) &&
          ...
          std::uniform_random_bit_generator<std::remove_reference_t<Gen>>
O sample( R&& r, O out, ranges::range_difference_t<R> n, Gen&& gen );  // (2) (since C++20)
```

1) Selects `M = min(n, last - first)` elements from the sequence
   `[``first``,``last``)` (without replacement) such that each possible *sample*
   has equal probability of appearance, and writes those selected elements into
   the range beginning at `out`.

The algorithm is *stable* (preserves the relative order of the selected
   elements) only if `I` models `std::forward_iterator`.
...
- **first1, last1** — the range from which to make the sampling (*the
  population*)
...
---
*Source: https://en.cppreference.com/w/cpp/algorithm/ranges/sample*
