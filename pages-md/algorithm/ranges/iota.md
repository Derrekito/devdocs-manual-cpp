...
using iota_result = ranges::out_value_result<O, T>;  // (3) (since C++23)
```
...
Uses the `vector` of iterators (`std::vector<std::list<T>::iterator`) as a proxy
to shuffle the elements of the `std::list`, because `ranges::shuffle` cannot be
applied to the `std::list` directly.
