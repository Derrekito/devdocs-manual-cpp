...
using set_symmetric_difference_result = ranges::in_in_out_result<I1, I2, O>;  // (3) (since C++20)
```
...
beginning at `result`. The resulting range is also sorted.

If some element is found `m` times in `[``first1``,``last1``)` and `n` times in
`[``first2``,``last2``)`, it will be copied to `result` exactly `│m - n│` times.
If `m > n`, then the last `m - n` of those elements are copied from
...
---
*Source: https://en.cppreference.com/w/cpp/algorithm/ranges/set_symmetric_difference*
