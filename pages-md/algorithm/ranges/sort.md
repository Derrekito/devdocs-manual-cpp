...
Note that typical implementations use Introsort. See also the implementation in
MSVC STL and libstdc++.
...
instead uses `ranges::iter_swap` (which performs ADL for `iter_swap`, unlike
`std::iter_swap`)
