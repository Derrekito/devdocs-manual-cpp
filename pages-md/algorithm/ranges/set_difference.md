...
using set_difference_result = ranges::in_out_result<I, O>;  // (3) (since C++20)
```

Copies the elements from the sorted input range `[``first1``,``last1``)` which
are not found in the sorted input range `[``first2``,``last2``)` to the output
...
- **first1, last1** — iterator-sentinel pair denoting the first sorted input
  range
- **first2, last2** — iterator-sentinel pair denoting the second sorted input
  range
...
---
*Source: https://en.cppreference.com/w/cpp/algorithm/ranges/set_difference*
