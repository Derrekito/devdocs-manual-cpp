...
1) Returns `true` if all elements in the range `[``first``,``last``)` that
   satisfy the predicate `pred` after projection appear before all elements that
   don't. Also returns `true` if `[``first``,``last``)` is empty.
...
`true` if the range `[``first``,``last``)` is empty or is partitioned by `pred`,
`false` otherwise.
