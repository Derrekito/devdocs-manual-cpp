...
1) Let `N1` and `N2` be `ranges::distance(first1, last1)` and
   `ranges::distance(first2, last2)` respectively. If `N1 < N2`, returns
   `false`. Otherwise, returns `true` if only if every element in the range
   `[``first2``,``last2``)` is equal to the corresponding element in `[``first1
   + N1 - N2``,``last1``)`. Comparison is done by applying the binary predicate
   `pred` to elements in two ranges projected by `proj1` and `proj2`
   respectively.
...
`true` if the second range matches the suffix of the first range, `false`
otherwise.
...
iterator-sentinel type pairs model `sized_sentinel_for`, or both range types
model `sized_range`) and `N1 < N2`, the time complexity is constant.
