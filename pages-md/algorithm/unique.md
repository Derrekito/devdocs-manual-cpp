...
ForwardIt unique( ExecutionPolicy&& policy, ForwardIt first, ForwardIt last );  // (2) (since C++17)
template< class ForwardIt, class BinaryPredicate >
ForwardIt unique( ForwardIt first, ForwardIt last, BinaryPredicate p );  // (until C++20)
template< class ForwardIt, class BinaryPredicate >
...
elements to be erased are overwritten.
...
of the container is unchanged. Iterators in `[``r``,``last``)` (if any), where
`r` is the return value, are still dereferenceable, but the elements themselves
have unspecified values. A call to `unique` is typically followed by a call to a
container's `erase` member function, which erases the unspecified values and
reduces the *physical* size of the container to match its new *logical* size.
...
previously published C++ standards.
  ...
  LWG 202 | C++98 | the behavior was unclear if the elements are compared using
      a non-equivalence relation | the behavior is undefined in this case
