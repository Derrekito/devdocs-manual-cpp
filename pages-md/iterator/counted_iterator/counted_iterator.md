...
1) Default constructor. Value-initializes the underlying iterator and
   initialized the underlying *length* with `​0​`. Operations on the resulting
   iterator have defined behavior if and only if the corresponding operations on
...
2) The underlying iterator is initialized with `std::move(x)` and the underlying
   *length* is initialized with `n`. The behavior is undefined if `n` is
   negative.
...
---
*Source: https://en.cppreference.com/w/cpp/iterator/counted_iterator/counted_iterator*
