...
1) `unreachable_sentinel_t` is an empty class type that can be used to denote
   the “upper bound” of an unbounded interval.

2) `unreachable_sentinel` is a constant of type `unreachable_sentinel_t`.

### Non-member functions

- ****operator==** (C++20)** — compares an `unreachable_sentinel_t` with a value
  of any `weakly_incrementable` type (function template)
...
`unreachable_sentinel_t` can be compared with any `weakly_incrementable` type
...
`std::unreachable_sentinel_t` is an associated class of the arguments.
...
---
*Source: https://en.cppreference.com/w/cpp/iterator/unreachable_sentinel_t*
