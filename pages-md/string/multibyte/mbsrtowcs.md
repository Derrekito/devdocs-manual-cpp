...
conversion state described by `*ps`, from the array whose first element is
pointed to by `*src` to its wide character representation. If `dst` is not null,
converted characters are stored in the successive elements of the wchar_t array
...
- An invalid multibyte character (according to the current C locale) was
  ...
  multibyte character.
- The next wide character to be stored would exceed `len`. `src` is set to point
  at the beginning of the first unconverted multibyte character. This condition
  is not checked if `dst` is a null pointer.
...
`L'\0'`, written to the character array. If `dst` is a null pointer, returns the
number of wide characters that would have been written given unlimited length.
...
This function moves the `src` pointer to the end of the converted multibyte
string. This doesn't happen if `dst` is a null pointer.
...
---
*Source: https://en.cppreference.com/w/cpp/string/multibyte/mbsrtowcs*
