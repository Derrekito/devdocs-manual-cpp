...
is undefined if `dest` and `src` overlap.
...
is `1 + std::strxfrm(nullptr, src, 0)`.

This function is used when making multiple locale-dependent comparisons using
the same string or set of strings, because it is more efficient to use
`std::strxfrm` to transform all the strings just once, and subsequently compare
the transformed strings with `std::strcmp`.

### Parameters

- **dest** — pointer to the first element of the array where the transformed
  string will be written
- **src** — pointer to the first character of a null-terminated byte string to
  transform
...
---
*Source: https://en.cppreference.com/w/cpp/string/byte/strxfrm*
