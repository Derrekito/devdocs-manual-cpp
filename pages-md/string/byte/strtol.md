...
performed, and (if `str_end` is not a null pointer) the value of `str` is stored
in the object pointed to by `str_end`.
...
- If the converted value falls out of range of corresponding return type, a
  range error occurs (setting `errno` to `ERANGE`) and `LONG_MAX`, `LONG_MIN`,
