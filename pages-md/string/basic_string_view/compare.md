...
1) The length `rlen` of the sequences to compare is the smaller of `size()` and
   ...
   `traits::compare(data(), v.data(), rlen)`, and returns a value according to
   the following table:

  Condition | Result | Return value
  `Traits::compare(data(), v.data(), rlen) < 0` | `this` is *less* than `v` | `<
      0`
  `Traits::compare(data(), v.data(), rlen) == 0` | `size() < v.size()` | `this`
      is *less* than `v` | `< 0`
  `size() == v.size()` | `this` is *equal* to `v` | `​0​`
  `size() > v.size()` | `this` is *greater* than `v` | `> 0`
  `Traits::compare(data(), v.data(), rlen) > 0` | `this` is *greater* than `v` |
      `> 0`
...
Negative value if this view is less than the other character sequence, zero if
the both character sequences are equal, positive value if this view is greater
