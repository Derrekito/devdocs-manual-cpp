...
```cpp
template< class T >
concept signed_integral = std::integral<T> && std::is_signed_v<T>;  // (since C++20)
```
...
`signed_integral<T>` may be satisfied by a type that is not a signed integer
type, for example, `char` (on a system where `char` is signed).
