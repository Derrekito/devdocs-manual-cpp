...
`perms` satisfies the requirements of BitmaskType (which means the bitwise
...
permissions (as reported by `filesystem::status`) are a combination of some of
the following bits:
...
### Member constants
  ...
  `owner_all` | `0700` | `S_IRWXU` | File owner has read, write, and
      execute/search permissions Equivalent to `owner_read | owner_write |
      owner_exec`
  ...
  `group_all` | `070` | `S_IRWXG` | The file's user group has read, write, and
      execute/search permissions Equivalent to `group_read | group_write |
      group_exec`
  ...
  `others_all` | `07` | `S_IRWXO` | Other users have read, write, and
      execute/search permissions Equivalent to `others_read | others_write |
      others_exec`
  ...
  `set_uid` | `04000` | `S_ISUID` | Set user ID to file owner user ID on
      execution
  `set_gid` | `02000` | `S_ISGID` | Set group ID to file's user group ID on
      execution
  `sticky_bit` | `01000` | `S_ISVTX` | Implementation-defined meaning, but POSIX
      XSI specifies that when set on a directory, only file owners may delete
      files even if the directory is writeable to others (used with `/tmp`)
  `mask` | `07777` | All valid permission bits. Equivalent to `all | set_uid |
      set_gid | sticky_bit`
...
represent permissions:
  ...
  `unknown` | `0xFFFF` | Unknown permissions (e.g. when
      `filesystem::file_status` is created without permissions)

### Notes

Permissions may not necessarily be implemented as bits, but they are treated
...
distinction, setting any of the three write bits set all three).
