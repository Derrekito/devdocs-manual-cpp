...
functions report domain errors as specified in `math_errhandling`). Third-party
libraries, however, use this. For example, boost.math throws `std::domain_error`
if `boost::math::policies::throw_on_error` is enabled (the default setting).
...
3) Copy constructor. If `*this` and `other` both have dynamic type
   `std::domain_error` then `std::strcmp(what(), other.what()) == 0`. No
...
Because copying `std::domain_error` is not permitted to throw exceptions, this
...
dynamic type `std::domain_error` then `std::strcmp(what(), other.what()) == 0`
...
previously published C++ standards.
  ...
  LWG 471 | C++98 | the explanatory strings of `std::domain_error`'s copies were
      implementation-defined | they are the same as that of the original
      `std::domain_error` object
