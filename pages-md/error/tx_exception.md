...
```cpp
template< class T >
class tx_exception : public std::runtime_error;  // (TM TS)
```
...
1-3) Constructs the exception object with `what_arg` as explanatory string that
   can be accessed through `what()` and `value` as the object that can be
...
dynamic type `std::tx_exception<T>` then `std::strcmp(what(), other.what()) ==
0` after assignment.
