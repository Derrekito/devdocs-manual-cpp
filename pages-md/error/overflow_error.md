...
arithmetic overflow errors (that is, situations where a result of a computation
...
boost.math throws `std::overflow_error` if
...
dynamic type `std::overflow_error` then `std::strcmp(what(), other.what()) == 0`
...
previously published C++ standards.
  ...
  LWG 471 | C++98 | the explanatory strings of `std::overflow_error`'s copies
      were implementation-defined | they are the same as that of the original
      `std::overflow_error` object
