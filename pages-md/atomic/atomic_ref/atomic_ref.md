...
1) Constructs an `atomic_ref` object referencing the object `obj`. The behavior
   is undefined if `obj` is not aligned to `required_alignment`.
...
final sum is printed. Non-atomic access may "loss" the results of some
operations due to data-races.
