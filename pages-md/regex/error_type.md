...
constexpr error_type error_stack =      /* unspecified */;  // (since C++11) (until C++17)
...
inline constexpr error_type error_stack =      /* unspecified */;  // (since C++17)
```
...
- **`error_collate`** — the expression contains an invalid collating element
  name
...
- **`error_brack`** — the expression contains mismatched square brackets (`'['`
  and `']'`)
...
- **`error_badrepeat`** — `'*'`, `'?'`, `'+'` or `'{'` was not preceded by a
...
- **regex_error (C++11)** — reports errors generated by the regular expressions
  library (class)
