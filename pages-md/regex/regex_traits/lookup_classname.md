...
character class in the currently imbued locale (that is, the string between `[:`
and `:]` in regular expressions), returns the implementation-defined value
representing this character class. Otherwise, returns zero.
...
to std::regex_traits<>::lookup_classname() with `[``first``,``last``)`
indicating the string `"lower"` and `icase == true`. This call returns the same
bitmask as the call generated by the regex `[:alpha:]` with `icase == false`.
...
classifications obtained by the `std::ctype` facet of the imbued locale, as
follows:
...
The classification returned for the string `"w"` may be exactly the same as
`"alnum"`, in which case `isctype()` adds `'_'` explicitly.
...
- **isctype** — indicates membership in a character class (public member
  function)
