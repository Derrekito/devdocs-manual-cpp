...
collating element in the currently imbued locale, returns the name of that
collating element. Otherwise, returns an empty string.

Collating elements are the symbols found in POSIX regular expressions between
`[.` and `.]`. For example, `[.a.]` matches the character `a` in the C locale.
`[.tilde.]` matches the character `~` in the C locale as well. `[.ch.]` matches
the digraph `ch` in Czech locale, but generates `std::regex_error` with error
code `std::regex_constants::error_collate` in most other locales.
...
---
*Source: https://en.cppreference.com/w/cpp/regex/regex_traits/lookup_collatename*
