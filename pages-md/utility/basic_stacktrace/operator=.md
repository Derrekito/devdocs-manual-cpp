...
If
   ...
   If the allocator of `*this` after assignment would compare unequal to its old
   value, the old allocator is used to deallocate the memory, then the new
   allocator is used to allocate it before copying the entries. Otherwise, the
   memory owned by `*this` may be reused when possible.
...
If
   ...
   assign each entries individually, allocating additional memory using its own
   allocator as needed.
...
---
*Source: https://en.cppreference.com/w/cpp/utility/basic_stacktrace/operator%3D*
