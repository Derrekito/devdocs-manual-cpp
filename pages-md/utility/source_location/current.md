...
Constructs a new `source_location` object corresponding to the location of the
call site.
...
If `current()` is invoked directly (via a function call that names `current()`),
it returns a `source_location` object with implementation-defined values
representing the location of the call. The values should be affected by the
`#line` preprocessor directive in the same manner as the predefined macros
...
If `current()` is used in a default argument, the return value corresponds to
