...
`std::tuple_element` interacts with the core language: it can provide structured
...
Users may specialize `std::tuple_element` for program-defined types to make them
tuple-like.

In normal cases where the `get` functions returns reference members or reference
to subobjects, only specializations for cv-unqualified types are needed to be
customized.
