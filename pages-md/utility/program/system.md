...
the parameter `command`. Returns an implementation-defined value (usually the
...
Implementation-defined value. If `command` is a null pointer, returns a nonzero
value if and only if the command processor exists.

### Notes

On POSIX systems, the return value can be decomposed using `WEXITSTATUS` and
`WSTOPSIG`.
...
`std::system`, if the spawned process performs any screen I/O.
...
---
*Source: https://en.cppreference.com/w/cpp/utility/program/system*
