...
```cpp
void swap( optional& other ) noexcept(/* see below */);  // (since C++17) (until C++20)
...
- If only one of `*this` and `other` contains a value (let's call this object
  ...
  from `std::move(*in)`, followed by destruction of the contained value of `in`
...
- If both `*this` and `other` contain values, the contained values are exchanged
  by calling `using std::swap; swap(**this, *other)`. `T` lvalues must satisfy
  Swappable.
...
and `other` are determined by the exception safety guarantees of `swap` of type
`T` or `T`'s move constructor, whichever is called. For both `*this` and
`other`, if the object contained a value, it is left containing a value, and the
other way round.
