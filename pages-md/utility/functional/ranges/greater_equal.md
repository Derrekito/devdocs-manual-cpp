...
- **operator()** — checks if the first argument is *greater* than or *equal* to
  the second (public member function)
...
Unlike `std::greater_equal`, `std::ranges::greater_equal` requires all six
comparison operators `<`, `<=`, `>`, `>=`, `==` and `!=` to be valid (via the
...
---
*Source: https://en.cppreference.com/w/cpp/utility/functional/ranges/greater_equal*
