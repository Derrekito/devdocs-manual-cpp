...
- **(constructor)** — constructs a new `bad_variant_access` object (public
  member function)
- **operator=** — replaces the `bad_variant_access` object (public member
  function)
...
dynamic type `std::bad_variant_access` then `std::strcmp(what(), other.what())
== 0` after assignment.
