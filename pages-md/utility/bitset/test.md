...
Unlike `operator[]`, performs a bounds check and throws `std::out_of_range` if
`pos` does not correspond to a valid position in the bitset.
...
`std::out_of_range` if `pos` does not correspond to a valid position within the
bitset.
