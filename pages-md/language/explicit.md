...
1) Specifies that a constructor or conversion function(since C++11)or deduction
   ...
   conversions and copy-initialization.

2) The `explicit` specifier may be used with a constant expression. The function
is explicit if and only if that constant expression evaluates to `true`.
*(since C++20)*

The explicit specifier may only appear within the decl-specifier-seq of the
declaration of a constructor or conversion function(since C++11) within its
class definition.
...
Both constructors (other than copy/move) and user-defined conversion functions
may be function templates; the meaning of `explicit` does not change.
