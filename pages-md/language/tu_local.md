...
Translation-unit-local (TU-local) entities are introduced to prevent entities
that are supposed to be local (not used in any other translation unit) being
exposed and used in other translation units.

An example from Understanding C++ Modules: Part 2 illustrates the problem of not
constraining exposures:
...
1. a type, function, variable, or template that has a name with internal
   linkage, or does not have a name with linkage and is declared, or introduced
   by a lambda expression, within the definition of a TU-local entity,
1. a type with no name that is defined outside a class-specifier, function body,
   or initializer or is introduced by a defining-type-specifier (type-specifier,
   class-specifier or enum-specifier) that is used to declare only TU-local
   entities,
1. a specialization of a TU-local template,
1. a specialization of a template with any TU-local template argument, or
1. a specialization of a template whose (possibly instantiated) declaration is
   an exposure (defined below).
...
1. it is, or is a pointer to, a TU-local function or the object associated with
   a TU-local variable, or
1. it is an object of class or array type and any of its subobjects or any of
   the objects or functions to which its non-static data members of reference
   type refer is TU-local and is usable in constant expressions.
...
1. E is not a function or function template and D contains an id-expression,
   type-specifier, nested-name-specifier, template-name, or concept-name
   denoting E, or
1. E is a function or function template and D contains an expression that names
   E or an id-expression that refers to a set of overloads that contains E.
...
1. the function-body for a non-inline function or function template (but not the
   deduced return type for a (possibly instantiated) definition of a function
   with a declared return type that uses a placeholder type),
1. the initializer for a variable or variable template (but not the variable’s
   type),
1. friend declarations in a class definition, and
1. any reference to a non-volatile const object or reference with internal or no
   linkage initialized with a constant expression that is not an odr-use,
...
private-module-fragment, if any) or module partition is an exposure, the program
is ill-formed. Such a declaration in any other context is deprecated.
...
ill-formed. A declaration instantiated for a template specialization appears at
the point of instantiation of the specialization.
...
---
*Source: https://en.cppreference.com/w/cpp/language/tu_local*
