...
Converts the value `x` to a value of type `T`, clamping `x` between the minimum
and maximum values of type `T`.

It is a compile-time error if either `T` or `U` is not a signed or unsigned
integer type (including standard integer type and extended integer type).
...
---
*Source: https://en.cppreference.com/w/cpp/numeric/saturate_cast*
