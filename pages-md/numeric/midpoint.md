...
2) This overload participates in overload resolution only if `T` is an object
   type. Use of this overload is ill-formed if `T` is an incomplete type.
...
2) If `a` and `b` point to, respectively, `x[i]` and `x[j]` of the same array
   object `x` (for the purpose of pointer arithmetic), returns a pointer to `x[i
   + (j - i) / 2]` (or, equivalently `x[std::midpoint(i, j)]`) where the
   division rounds towards zero. If `a` and `b` do not point to elements of the
   same array object, the behavior is undefined.
...
because there may be some platforms where creating an array with number of
elements greater than `PTRDIFF_MAX` is possible, and `b - a` may result in
undefined behavior even if both `b` and `a` point to elements in the same array.
