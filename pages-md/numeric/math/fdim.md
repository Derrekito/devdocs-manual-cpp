...
1-3) Returns the positive difference between `x` and `y`, that is, if `x > y`,
   returns `x - y`, otherwise (i.e. if `x <= y`) returns `+0`. The library
   provides overloads of `std::fdim` for all cv-unqualified floating-point types
   as the type of the parameters.(since C++23)
...
- If `num1` or `num2` has type long double, then `std::fdim(num1, num2)` has the
  same effect as `std::fdim(static_cast<long double>(num1), static_cast<long
  double>(num2))`.
...
- Otherwise, if `num1` or `num2` has type float, then `std::fdim(num1, num2)`
  has the same effect as `std::fdim(static_cast<float>(num1),
  static_cast<float>(num2))`.
*(until C++23)*

If `num1` and `num2` have arithmetic types, then `std::fdim(num1, num2)` has the
same effect as `std::fdim(static_cast</* common-floating-point-type */>(num1),
