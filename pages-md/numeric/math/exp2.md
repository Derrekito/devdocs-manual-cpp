...
/* floating-point-type */
            exp2 ( /* floating-point-type */ num );  // (since C++23) (constexpr since C++26)
...
1-3) Computes 2 raised to the given power `num`. The library provides overloads
   of `std::exp2` for all cv-unqualified floating-point types as the type of the
...
type, `std::exp2(num)` has the same effect as
