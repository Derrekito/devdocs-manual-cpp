...
/* floating-point-type */
            erfc ( /* floating-point-type */ num );  // (since C++23) (constexpr since C++26)
...
1-3) Computes the complementary error function of `num`, that is `1.0 -
   ...
   provides overloads of `std::erfc` for all cv-unqualified floating-point types
...
\(\frac{2}{\sqrt{\pi} }\int_{num}^{\infty}{e^{-{t^2}
}\mathsf{d}t}\)2/√π∫∞num*e*-t2d*t* or \({\small
1-\operatorname{erf}(num)}\)1-erf(num), is returned.
...
type, `std::erfc(num)` has the same effect as
