...
/* floating-point-type */
            cos ( /* floating-point-type */ num );  // (since C++23) (constexpr since C++26)
...
1-3) Computes the cosine of `num` (measured in radians). The library provides
   overloads of `std::cos` for all cv-unqualified floating-point types as the
...
C, but it is defined as a domain error in POSIX.
...
type, `std::cos(num)` has the same effect as
