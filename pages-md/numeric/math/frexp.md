...
1-3) Decomposes given floating point value `num` into a normalized fraction and
   an integral power of two. The library provides overloads of `std::frexp` for
...
the range `(-1, -0.5], [0.5, 1)` and stores an integer value in `*exp` such that
x×2(*exp)== num.
...
- If `num` is NaN, NaN is returned, and an unspecified value is stored in
  `*exp`.
...
On a binary system (where `FLT_RADIX` is `2`), `std::frexp` may be implemented
as
...
type, `std::frexp(num, exp)` has the same effect as
