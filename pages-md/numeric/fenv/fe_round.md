...
expression, which can be used with `std::fesetround` and `std::fegetround` to
indicate one of the supported floating-point rounding modes. The implementation
may define additional rounding mode constants in `<cfenv>`, which should all
begin with `FE_` followed by at least one uppercase letter. Each macro is only
...
- results of floating-point arithmetic operators in expressions executed at
  compile time (always to nearest)
...
Compilers that do not support the pragma may offer their own ways to support
current rounding mode. For example Clang and GCC have the option
`-frounding-math` intended to disable optimizations that would change the
meaning of rounding-sensitive code.
...
---
*Source: https://en.cppreference.com/w/cpp/numeric/fenv/FE_round*
