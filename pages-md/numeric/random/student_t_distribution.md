...
density function: \(p(x|n) = \frac{1}{\sqrt{n\pi} } \cdot
...
used when estimating the *mean* of an unknown normally distributed value given n
+ 1 independent measurements, each with additive errors of unknown standard
deviation, as in physical measurements. Or, alternatively, when estimating the
unknown mean of a normal distribution with unknown standard deviation, given n +
1 samples.
...
- **n** — returns the *n* distribution parameter (degrees of freedom) (public
  member function)
