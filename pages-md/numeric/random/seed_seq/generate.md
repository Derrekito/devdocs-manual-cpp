...
232, based on the data originally provided in the constructor of this
`seed_seq`. The produced values are distributed over the entire 32-bit range
even if the initial values were strongly biased.
...
incorporating the improvements made by Mutsuo Saito in 2007)
...
where `p = (n - t) / 2`
and `q = p + t`
and `t = (n >= 623) ? 11 : (n >= 68) ? 7 : (n >= 39) ? 5 : (n >= 7) ? 3 : (n -
1) / 2`
...
where all calculations are performed modulo 232 and where the indexing of the
output range (`begin[x]`) is taken modulo n.

### Parameters

- **begin, end** — mutable random-access iterators whose
  `std::iterator_traits<>::value_type` is an unsigned integer type suitable for
...
---
*Source: https://en.cppreference.com/w/cpp/numeric/random/seed_seq/generate*
