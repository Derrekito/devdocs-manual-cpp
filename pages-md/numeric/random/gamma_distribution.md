...
probability density function: \(\mathsf{p}(x\mid\alpha,\beta) =
\frac{e^{-x/\beta} }{\beta^\alpha\cdot\Gamma(\alpha)}\cdot x^{\alpha-1}
\)P(x|α,β) =

e-x/β/βα · Γ(α) · xα-1

where α is known as the *shape* parameter and β is known as the *scale*
parameter. The shape parameter is sometimes denoted by the letter k and the
scale parameter is sometimes denoted by the letter θ.
...
- **alphabeta (C++11)** — returns the distribution parameters (public member
  function)
