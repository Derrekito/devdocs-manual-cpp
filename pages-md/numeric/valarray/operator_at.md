...
object. The non-const overloads return classes holding references to the array
elements.

The selected elements(s) must exist:

- for overloads (1,2), if `pos` is not less than `size()`, the behavior is
  undefined; and
- for overloads (3-10), if the argument does not specify a valid subset of
...
= x;` is an error because `std::mask_array` (the type of `v[v == n]`) does not
have operator[].
...
previously published C++ standards.
  ...
  LWG 389 | C++98 | the return type of overload (1) was `T` | corrected to const
      T&
  LWG 430 | C++98 | the behavior was unclear for overloads (3-10) if an invalid
      subset is specified | the behavior is undefined in this case
