...
`std::slice_array` is a helper template used by the valarray subscript operator
with `std::slice` argument. It has reference semantics to a subset of the array
specified by the `std::slice` object.
...
-
  **operator+=operator-=operator*=operator/=operator%=operator&=operator|=operator^=operator<<=operator>>=**
  — performs arithmetic operation on the array referred by slice. (public member
  function)
...
---
*Source: https://en.cppreference.com/w/cpp/numeric/valarray/slice_array*
