...
long double laguerre ( unsigned int n, long double x );  // (since C++17) (until C++23)
...
1-3) Computes the non-associated Laguerre polynomials of the degree `n` and
   argument `x`. The library provides overloads of `std::laguerre` for all
...
If no errors occur, value of the nonassociated Laguerre polynomial of `x`, that
is ex/n!dn/dxn(xn*e*-x), is returned.
...
type, `std::laguerre(int_num, num)` has the same effect as
`std::laguerre(int_num, static_cast<double>(num))`.
...
### External links
  ...
  Weisstein, Eric W. "Laguerre Polynomial." From MathWorld — A Wolfram Web
      Resource.
