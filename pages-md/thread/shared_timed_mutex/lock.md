...
If `lock` is called by a thread that already owns the `mutex` in any mode
...
mode: the shared lock has to be relinquished with `unlock_shared()` before
exclusive ownership may be obtained with `lock()`. boost::upgrade_mutex may be
...
---
*Source: https://en.cppreference.com/w/cpp/thread/shared_timed_mutex/lock*
