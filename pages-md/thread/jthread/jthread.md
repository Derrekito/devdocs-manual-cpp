...
explicit jthread( Function&& f, Args&&... args );  // (3) (since C++20)
...
2) Move constructor. Constructs the `std::jthread` object to represent the
   thread of execution that was represented by `other`. After this call `other`
...
3) Creates new `std::jthread` object and associates it with a thread of
   ...
   auto(std::forward<Args>(args))...)` (since C++23) if the expression above is
   well-formed, otherwise starts executing:
