...
If any threads are waiting on `*this`, calling `notify_one` unblocks one of the
waiting threads.
...
notified thread would immediately block again, waiting for the notifying thread
to release the lock. However, some implementations (in particular many
...
thread's condition variable. A spurious wakeup after mutex unlock but before
...
---
*Source: https://en.cppreference.com/w/cpp/thread/condition_variable_any/notify_one*
