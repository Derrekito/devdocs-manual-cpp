...
shared_lock( mutex_type& m,
             const std::chrono::duration<Rep,Period>& timeout_duration );  // (7) (since C++14)
template< class Clock, class Duration >
shared_lock( mutex_type& m,
             const std::chrono::time_point<Clock,Duration>& timeout_time );  // (8) (since C++14)
```
...
2) Move constructor. Initializes the `shared_lock` with the contents of `other`.
   Leaves `other` with no associated mutex.
...
8) Tries to lock the associated mutex in shared mode by calling
   ...
   first. May block for longer than until `timeout_time` has been reached. The
   behavior is undefined if `Mutex` does not meet the SharedTimedLockable
   requirements.
