...
has elapsed (timeout) or the lock is acquired (owns the mutex), whichever comes
...
duration. If an implementation uses a `std::system_clock` instead, the wait time
...
calls to `try_lock_for` increment the ownership count: the mutex will only be
...
`try_lock_for` will return `false` if this number is exceeded.
...
Any exception thrown by `timeout_duration` (durations provided by the standard
library never throw).
...
---
*Source: https://en.cppreference.com/w/cpp/thread/recursive_timed_mutex/try_lock_for*
