...
struct adopt_lock_t { explicit adopt_lock_t() = default; };  // (since C++11)
```

`std::defer_lock_t`, `std::try_to_lock_t` and `std::adopt_lock_t` are empty
class tag types used to specify locking strategy for `std::unique_lock` and
`std::shared_lock`.
...
---
*Source: https://en.cppreference.com/w/cpp/thread/lock_tag_t*
