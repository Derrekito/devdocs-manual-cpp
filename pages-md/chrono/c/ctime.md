...
resulting string has the following format:
...
- `Www` - the day of the week (one of `Mon`, `Tue`, `Wed`, `Thu`, `Fri`, `Sat`,
  `Sun`).
- `Mmm` - the month (one of `Jan`, `Feb`, `Mar`, `Apr`, `May`, `Jun`, `Jul`,
  `Aug`, `Sep`, `Oct`, `Nov`, `Dec`).
...
This function returns a pointer to static data and is not thread-safe. In
addition, it modifies the static `std::tm` object which may be shared with
`std::gmtime` and `std::localtime`. POSIX marks this function obsolete and
recommends `std::strftime` instead.
