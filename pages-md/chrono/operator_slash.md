...
A plain integer is accepted if its meaning is unambiguous from the types of
other operands: `2005y/4/5` is allowed, but `5/April/2005` is not.
...
33,35) `std::chrono::year_month_weekday(std::chrono::year(y), mwd.month(),
   mwd.weekday_indexed())`
...
38,40) `std::chrono::year_month_weekday_last(std::chrono::year(y), mwdl.month(),
   mwdl.weekday_last())`
