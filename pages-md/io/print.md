...
template< class... Args >
void print( std::format_string<Args...> fmt, Args&&... args );  // (2) (since C++23)
```

Format `args` according to the format string `fmt`, and print the result to a
stream.
...
2) same as (1) with the standard C output stream `stdout` bound to `stream`,
   i.e. `std::print(stdout, fmt, args...);`
