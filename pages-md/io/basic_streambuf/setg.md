...
Sets the values of the pointers defining the get area. Specifically, after the
call `eback() == gbeg`, `gptr() == gcurr`, `egptr() == gend`.

### Parameters

- **gbeg** — pointer to the new beginning of the get area
- **gcurr** — pointer to the new current character (*get pointer*) in the get
  area
