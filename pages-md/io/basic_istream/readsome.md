...
The extracted characters are stored into the character array pointed to by `s`.

Behaves as UnformattedInputFunction. After constructing and checking the sentry
object,

- If `rdbuf()->in_avail() == -1`, calls `setstate(eofbit)` and extracts no
  characters.
...
The behavior of this function is highly implementation-specific. For example,
when used with `std::ifstream`, some library implementations fill the underlying
`filebuf` with data as soon as the file is opened (and `readsome()` on such
implementations reads data, potentially, but not necessarily, the entire file),
while other implementations only read from file when an actual input operation
is requested (and `readsome()` issued after file opening never extracts any
characters). Likewise, a call to `std::cin.readsome()` may return all pending
unprocessed console input, or may always return zero and extract no characters.
