...
The class `std::basic_ios` provides facilities for interfacing with objects that
have `std::basic_streambuf` interface. Several `std::basic_ios` objects can
refer to one actual `std::basic_streambuf` object.
...
- **init** — initializes a default-constructed `std::basic_ios` (protected
  member function)
...
Straightforward implementation of `std::basic_ios` stores only the following
members (which all depend on the template parameters and thus cannot be part of
`std::ios_base`):
...
LLVM libc++ stores 1 less member: it maintains the `rdbuf` pointer as a void*
member of `ios_base`.
