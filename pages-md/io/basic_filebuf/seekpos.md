...
Repositions the file pointer, if possible, to the position indicated by `sp`. If
the associated file is not open (`is_open() == false`), fails immediately.
...
If `sp` was not obtained by calling `seekoff()` or `seekpos()` on the same file,
the behavior is undefined.

### Parameters

- **sp** — file position obtained by `seekoff()` or `seekpos()` called earlier
  on the same file
...
previously published C++ standards.
  ...
  LWG 171 | C++98 | the sequence of the operations of reposition was not clear |
      made clear
