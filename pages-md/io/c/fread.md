...
`stream` as if by calling `std::fgetc` `size` times for each object, and storing
the results, in the order obtained, into the successive positions of `buffer`,
which is reinterpreted as an array of `unsigned char`. The file position
indicator for the stream is advanced by the number of characters read.
...
Number of objects read successfully, which may be less than `count` if an error
