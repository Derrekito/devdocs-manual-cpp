...
The class template `std::basic_ispanstream` implements input operations on
streams based on fixed buffers.

At the low level, the class essentially wraps a raw device implementation of
`std::basic_spanbuf` into a higher-level interface of `std::basic_istream`. The
...
- **(constructor) (C++23)** — constructs the `basic_ispanstream` (public member
  function)
- ****(destructor)** [virtual] (C++23)** — destroys the `basic_ispanstream`
  (virtual public member function)
- **operator= (C++23)** — move-assignments the `basic_ispanstream` (public
  member function)
