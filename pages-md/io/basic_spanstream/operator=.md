...
1) Move assigns the `other` to `*this`, effectively move-assigning both the
   `std::basic_iostream` base class subobject and the wrapped
   `std::basic_spanbuf`.

2) Copy assignment operator is deleted. `basic_spanstream` is not copy
   assignable.
...
---
*Source: https://en.cppreference.com/w/cpp/io/basic_spanstream/operator%3D*
