...
threads or logging activity of a program that may crash unexpectedly. An
explicit flush of `std::cout` is also necessary before a call to `std::system`,
if the spawned process performs any screen I/O. In most other usual interactive
...
call to `std::cout.flush()`. Use of `std::endl` in place of '\n', encouraged by
...
In those situations, unnecessary `endl` only degrades the performance of file
output, not standard output.
...
With '\n' instead of `endl`, the output would be the same, but may not appear in
real time.
