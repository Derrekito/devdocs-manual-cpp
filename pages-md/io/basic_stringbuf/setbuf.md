...
nothing, while some implementations clear the `std::string` member currently
used as the buffer and begin using the user-supplied character array of size
`n`, whose first element is pointed to by `s`, as the buffer and the
input/output character sequence.
...
`boost::basic_array` may be used to implement I/O buffering over a user-provided
char array in portable manner.
...
---
*Source: https://en.cppreference.com/w/cpp/io/basic_stringbuf/setbuf*
