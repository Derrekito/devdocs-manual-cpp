...
(`std::basic_filebuf`) with the high-level interface of (`std::basic_istream`).

A typical implementation of `std::basic_ifstream` holds only one non-derived
...
- **(destructor) [virtual] (implicitly declared)** — destructs the
  `basic_ifstream` and the associated buffer, closes the file (virtual public
  member function)
