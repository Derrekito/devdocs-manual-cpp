...
2) Copies all member objects from `other` to `*this` except for `rdstate()`, the
   exception mask, and `rdbuf()`. In particular, makes copies of the locale, the
   formatting flags, the contents of the arrays `std::ios_base::iword` and
   `std::ios_base::pword` (but not the `iword` and `pword` pointers themselves),
   the callbacks, and the tied stream.
...
provides a more fine-grained I/O state savers library for the same purpose.
...
previously published C++ standards.
  ...
  LWG 256 | C++98 | step 3 called the registered callbacks with the event type
      `copy_event`, which is not defined | corrected to `copyfmt_event`
  LWG 292 | C++98 | if `other` refers to the same object as `*this`, the member
      objects were still copied and the registered callbacks were still called |
