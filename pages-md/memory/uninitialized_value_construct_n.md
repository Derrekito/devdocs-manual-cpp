...
ForwardIt uninitialized_value_construct_n( ForwardIt first, Size n );  // (1) (since C++17)
...
1) Constructs `n` objects of type `typename
   iterator_traits<ForwardIt>::value_type` in the uninitialized storage starting
   at `first` by value-initialization, as if by `for (; n > 0; (void) ++first,
   --n) ::new (static_cast<void*>(std::addressof(*first))) typename
