...
using uninitialized_move_result = ranges::in_out_result<I, O>;  // (3) (since C++20)
```

1) Moves `N` elements from the input range `[``ifirst``,``ilast``)` to the
   output range `[``ofirst``,``olast``)` (that is an uninitialized memory area),
   where `N` is `min(ranges::distance(ifirst, ilast), ranges::distance(ofirst,
   olast))`.

The effect is equivalent to: for (; ifirst != ilast && ofirst != olast;
   ++ofirst, ++ifirst) ::new (static_cast<void*>(std::addressof(*ofirst)))
...
If an exception is thrown during the initialization then the objects that
   already constructed in `[``ofirst``,``olast``)` are destroyed in an
...
`ranges::uninitialized_move`, e.g. by using `ranges::copy_n`, if the value type
of the output range is TrivialType.
