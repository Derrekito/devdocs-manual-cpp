...
The expression `std::addressof(e)` is a constant subexpression, if `e` is an
lvalue constant subexpression.
*(since C++17)*
...
The implementation below is not `constexpr`, because `reinterpret_cast` is not
usable in a constant expression. Compiler support is needed (see below).

```cpp
// std::remove_reference is needed for explicitly specifying template arguments
...
libstdc++, LLVM libc++, Microsoft STL.
...
to C++14 mode as a defect report.
...
- **pointer_to [static]** — obtains a dereferenceable pointer to its argument
