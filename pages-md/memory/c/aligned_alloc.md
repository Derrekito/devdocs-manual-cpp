...
- `std::calloc`, `std::malloc`, `std::realloc`, `std::aligned_alloc`,
  `std::free`
...
`alignment` which is not valid or not supported by the implementation causes the
function to fail and return a null pointer (C11, as published, specified
undefined behavior in this case, this was corrected by DR460).
...
and not greater than `alignof(std::max_align_t)`, `aligned_alloc` may simply
call `std::malloc`.

Regular `std::malloc` aligns memory suitable for any object type with a
fundamental alignment. This function is useful for over-aligned allocations,
such as to SSE, cache line, or VM page boundary.
...
kind. Instead, MS CRT provides `_aligned_malloc` (to be freed with
`_aligned_free`).
