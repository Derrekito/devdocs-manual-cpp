...
struct hash<std::unique_ptr<T, Deleter>>;  // (since C++11)
```
...
`std::hash`) if `std::hash<typename std::unique_ptr<T,D>::pointer>` is enabled,
and is disabled otherwise.

When enabled, for a given `std::unique_ptr<T, D> p`, this specialization ensures
that `std::hash<std::unique_ptr<T, D>>()(p) == std::hash<typename
std::unique_ptr<T, D>::pointer>()(p.get())`.
...
---
*Source: https://en.cppreference.com/w/cpp/memory/unique_ptr/hash*
