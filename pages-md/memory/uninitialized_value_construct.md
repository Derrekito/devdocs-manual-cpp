...
void uninitialized_value_construct( ForwardIt first, ForwardIt last );  // (1) (since C++17)
template< class ExecutionPolicy, class ForwardIt >
void uninitialized_value_construct( ExecutionPolicy&& policy,
                                    ForwardIt first, ForwardIt last );  // (2) (since C++17)
```

1) Constructs objects of type `typename iterator_traits<ForwardIt>::value_type`
   in the uninitialized storage designated by the range `[``first``,``last``)`
   by value-initialization, as if by `for (; first != last; ++first) ::new
