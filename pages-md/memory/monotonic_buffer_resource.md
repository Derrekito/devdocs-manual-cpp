...
is no initial buffer, or if the buffer is exhausted, additional buffers are
obtained from an *upstream memory resource* supplied at construction. The size
of buffers obtained follows a geometric progression.

`monotonic_buffer_resource` is not thread-safe.

### Member functions

- **(constructor)** — constructs a `monotonic_buffer_resource` (public member
  function)
- **(destructor) [virtual]** — destroys a `monotonic_buffer_resource`, releasing
  all allocated memory (virtual public member function)
- **operator= [deleted]** — copy assignment operator is deleted.
  `monotonic_buffer_resource` is not copy assignable (public member function)
