...
Locates the first character in the character array `[``beg``,``end``)` that
satisfies the classification mask `m`, that is, the first character `c` such
that `table()[(unsigned char) c] & m` would return `true`.
