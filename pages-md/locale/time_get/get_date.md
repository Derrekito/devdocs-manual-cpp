...
1) Public member function, calls the protected virtual member function
   `do_get_date` of the most derived class.

2) Reads successive characters from the sequence `[``beg``,``end``)` and parses
   out the calendar date value using the default format expected by this locale,
   which is determined as `date_order()` Format `no_order` `"%m/%d/%y"` `dmy`
   `"%d/%m/%y"` `mdy` `"%m/%d/%y"` `ymd` `"%y/%m/%d"` `ydm` `"%y/%d/%m"`
...
For the alphabetic components of the default date format (if any), this function
is usually case-insensitive.

If a parsing error is encountered, most implementations of this function leave
`*t` unmodified.

The implementation may support other date formats besides the ones required by
the standard.
...
previously published C++ standards.
  ...
  LWG 248 | C++98 | `eofbit` was not set upon reaching the end iterator | sets
      `eofbit` if a valid date has not been read
