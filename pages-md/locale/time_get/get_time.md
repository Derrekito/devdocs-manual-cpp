...
1) Public member function, calls the protected virtual member function
   `do_get_time` of the most derived class.

2) Reads successive characters from the sequence `[``beg``,``end``)` and parses
   out the time value following the same rules as the format specifier
   `"%H:%M:%S"` as used by the functions `std::get_time`, `time_get::get`, and
   the POSIX function `strptime()`.
The parsed time is stored in the corresponding fields of the `std::tm` structure
pointed to by the argument `t`. If the end iterator is reached before a valid
time is read, the function sets `std::ios_base::eofbit` in `err`. If a parsing
error is encountered, the function sets `std::ios_base::failbit` in `err`.
...
- **str** — a stream object that this function uses to obtain locale facets when
  needed, e.g. `std::ctype` to skip whitespace
...
recognized as a part of a valid date.

### Notes

For the alphabetic components of the default time format (if any), this function
...
previously published C++ standards.
  ...
  LWG 248 | C++98 | `eofbit` was not set upon reaching the end iterator | sets
      `eofbit` if a valid time has not been read
  LWG 461 | C++98 | `do_get_time` needed to parse localized time representation
