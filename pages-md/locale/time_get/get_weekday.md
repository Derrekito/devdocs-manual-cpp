...
2) Reads successive characters from the sequence `[``beg``,``end``)` and parses
   out the weekday name (possibly abbreviated), using the default format for
   weekdays expected by this locale, which is the same format as `"%a"` as used
   by the functions `std::get_time`, `time_get::get`, and the POSIX function
   `strptime()`.
...
full name or finds a character that is not expected, in which case parsing fails
...
---
*Source: https://en.cppreference.com/w/cpp/locale/time_get/get_weekday*
