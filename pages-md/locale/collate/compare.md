...
2) Compares the character sequence `[``low1``,``high1``)` to the character
   ...
   returns `1` if the first string follows the second, `-1` if the first string
   precedes the second, zero if the two strings are equivalent.
...
`1` if the first string is greater than the second (that is, follows the second
in the collation order), `-1` if the first string is less than the second
(precedes the second in the collation order), zero if the two strings are
equivalent.

### Notes

When three-way comparison is not required (such as when providing a `Compare`
argument to standard algorithms such as `std::sort`), `std::locale::operator()`
