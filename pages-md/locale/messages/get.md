...
2) Obtains a message from the open message catalog `cat` using the values `set`,
   `msgid` and `dfault` in implementation-defined manner. If the expected
   message is not found in the catalog, returns a copy of `dfault`.
...
- **dfault** — the string to look up in the catalog (if the catalog uses string
  look-up) and also the string to return in case of a failure
...
`catgets()`, and the parameters `set`, `msgid`, and `dfault` are passed to
`catgets()` as-is. In GNU libstdc++, this function ignores `set` and `msgid` and
simply calls GNU `gettext(dfault)` in the required locale.
