...
Sorts the elements and preserves the order of equivalent elements. No references
...
1) Approximately \(\scriptsize N \cdot log(N)\)N·log(N) comparisons using
   `operator<`.
...
This function also differs from `std::sort` in that it does not require the
element type of the `list` to be swappable, preserves the values of all
iterators, and performs a stable sort.
...
---
*Source: https://en.cppreference.com/w/cpp/container/list/sort*
