...
Moves elements from another `forward_list` to `*this`. The elements are inserted
after the element pointed to by `pos`.

No elements are copied. No iterators or references become invalidated. The
iterators to the moved elements now refer into `*this`, not into `other`.

1,2) Moves all elements from `other` into `*this`. The container `other` becomes
   empty after the operation.

3,4) Moves the element pointed to by the iterator following `it` from `other`
   into `*this`. Has no effect if `pos == it` or if `pos == ++it`.
...
- for overloads (3,4), the iterator following `it` is not a dereferenceable
  iterator into `other`, or
- for overloads (5,6),
...
previously published C++ standards.
  ...
  LWG 2045 | C++11 | O(1) splicing could not be guaranteed if `get_allocator()
