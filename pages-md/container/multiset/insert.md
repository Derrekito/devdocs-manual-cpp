...
1,2) Inserts `value`. If the container has elements with equivalent key, inserts
...
7) If `nh` is an empty node handle, does nothing. Otherwise, inserts the element
   owned by `nh` into the container and returns an iterator pointing at the
   inserted element. If a range containing elements with keys equivalent to
   `nh.key()` exists in the container, the element is inserted at the end of
   that range. The behavior is undefined if `nh` is not empty and
...
8) If `nh` is an empty node handle, does nothing and returns the end iterator.
   ...
   the iterator pointing to the element with key equivalent to `nh.key()`. The
   element is inserted as close as possible to the position just prior to `pos`.
...
1-4,7,8) If an exception is thrown by any operation, the insertion has no
   effect.
...
3,4,8) Amortized constant if the insertion happens in the position just before
   `pos`, `O(log(size()))` otherwise.
...
previously published C++ standards.
  ...
  LWG 264 | C++98 | the complexity of overload (5) was required to be linear if
      ...
      linear requirement in this special case
  LWG 371 | C++98 | the order of equivalent elements was not guaranteed to be
      preserved | required to be preserved
