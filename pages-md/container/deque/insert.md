...
1,2) Constant plus linear in the lesser of the distances between `pos` and
...
4) Linear in `std::distance(first, last)` plus linear in the lesser of the
   distances between `pos` and either of the ends of the container.

5) Linear in `ilist.size()` plus linear in the lesser of the distances between
...
function has no effect (strong exception guarantee). Otherwise, if an exception
is thrown by the move constructor of a non-CopyInsertable `T`, the effects are
unspecified.
*(since C++11)*
...
previously published C++ standards.
  ...
  LWG 247 | C++98 | the complexity was only specified for inserting a single
      element | also specified for inserting multiple elements
...
---
*Source: https://en.cppreference.com/w/cpp/container/deque/insert*
