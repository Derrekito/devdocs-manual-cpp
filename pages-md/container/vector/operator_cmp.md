...
bool operator==( const std::vector<T, Alloc>& lhs,
                 const std::vector<T, Alloc>& rhs );  // (until C++20)
template< class T, class Alloc >
constexpr bool operator==( const std::vector<T, Alloc>& lhs,
                           const std::vector<T, Alloc>& rhs );  // (since C++20)
template< class T, class Alloc >
bool operator!=( const std::vector<T, Alloc>& lhs,
                 const std::vector<T, Alloc>& rhs );  // (2) (until C++20)
template< class T, class Alloc >
bool operator<( const std::vector<T, Alloc>& lhs,
                const std::vector<T, Alloc>& rhs );  // (3) (until C++20)
template< class T, class Alloc >
bool operator<=( const std::vector<T, Alloc>& lhs,
                 const std::vector<T, Alloc>& rhs );  // (4) (until C++20)
template< class T, class Alloc >
bool operator>( const std::vector<T, Alloc>& lhs,
                const std::vector<T, Alloc>& rhs );  // (5) (until C++20)
template< class T, class Alloc >
bool operator>=( const std::vector<T, Alloc>& lhs,
                 const std::vector<T, Alloc>& rhs );  // (6) (until C++20)
template< class T, class Alloc >
constexpr /* see below */ operator<=>( const std::vector<T, Alloc>& lhs,
                                       const std::vector<T, Alloc>& rhs );  // (7) (since C++20)
```
...
`vector`s with a function object performing *synthesized three-way comparison*
...
1) `true` if the contents of the `vector`s are equal, `false` otherwise.

2) `true` if the contents of the `vector`s are not equal, `false` otherwise.
...
previously published C++ standards.
  ...
  LWG 469 | C++98 | `std::vector<bool>` specialized the comparison operators |
      removed the specializations

---
*Source: https://en.cppreference.com/w/cpp/container/vector/operator_cmp*
