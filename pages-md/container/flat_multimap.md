...
entries with the same key value. Keys are sorted by using the comparison
function `Compare`.

The class template `flat_multimap` acts as a wrapper to the two underlying
...
A flat multimap supports most AssociativeContainer's operations that use equal
keys.
...
- **(destructor)** — destructs the `flat_multimap` (public member function)
...
- **operator==operator<=> (C++23)** — lexicographically compares the values of
  two `flat_multimaps` (function template)
- **std::swap(std::flat_multimap) (C++23)** — specializes the `std::swap`
  algorithm (function template)
- **erase_if(std::flat_multimap) (C++23)** — erases all elements satisfying
...
---
*Source: https://en.cppreference.com/w/cpp/container/flat_multimap*
