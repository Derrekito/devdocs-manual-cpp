...
map( InputIt, InputIt, Comp = Comp(), Alloc = Alloc() )
    -> map<iter_key_t<InputIt>, iter_val_t<InputIt>, Comp, Alloc>;  // (1) (since C++17)
template< class Key,
          class T,
...
map( std::initializer_list<std::pair<Key, T>>, Alloc )
    -> map<Key, T, std::less<Key>, Alloc>;  // (4) (since C++17)
...
map( std::from_range_t, R&&, Compare = Compare(), Alloc = Alloc() )
    -> map<range_key_t<R>, range_mapped_t<R>, Compare, Alloc>;  // (5) (since C++23)
template< ranges::input_range R, class Alloc >
map( std::from_range_t, R&&, Alloc )
    -> map<range_key_t<R>, range_mapped_t<R>, std::less<range_key_t<R>>, Alloc>;  // (6) (since C++23)
...
1-4) These deduction guides are provided for `map` to allow deduction from an
   iterator range (overloads (1,3)) and `std::initializer_list` (overloads
   (2,4)).
...
---
*Source: https://en.cppreference.com/w/cpp/container/map/deduction_guides*
